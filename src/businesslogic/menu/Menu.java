...
public class Menu {
    ...
    public static Menu loadMenuById(int id_menu){
        Menu menu = new Menu(id_menu);
        String query = "SELECT * FROM menus WHERE id=" + id_menu;

        PersistenceManager.executeQuery(query, new ResultHandler() {
            @Override
            public void handle(ResultSet rs) throws SQLException {
                menu.title = rs.getString("title");
                User user = User.loadUserById(rs.getInt("owner_id"));
                menu.owner = user;
        ...
        // load sections
        menu.sections = Section.loadSectionsFor(id_menu);

        // load free items
        menu.freeItems = MenuItem.loadItemsFor(id_menu, 0);
    ...
    public ObservableList<Recipe> getRecipes() {
        ...
        for (MenuItem item : freeItems) {
            Recipe r = item.getItemRecipe();
            recipes.add(r);
        }
        for(Section sec: sections){
            ObservableList<MenuItem> menuItems = sec.getItems();
